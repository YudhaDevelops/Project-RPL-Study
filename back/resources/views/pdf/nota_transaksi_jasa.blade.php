...
<head>
    ...
    <style>
        ...
        .invoice-box table tbody tr.item td.harga {
            text-align: center;
        }
    ...
    <div class="invoice-box" id="print-area-2">
        <table>
            ...
            <tr class="heading">
                ...
                <td style="text-align: left">Jasa Yang Digunakan</td>
                <td>Nama Hewan</td>
                <td>Price</td>
            </tr>
            @if (isset($pembayaran) && $pembayaran != null)
                <?php $i = 1; ?>
                @foreach ($pembayaran as $bayar)
                    <tr class="item">
                        ...
                        <td style="text-align: left">
                              {{ $bayar->nama_jasa }} <br>
                            @if ($bayar->nama_jasa == 'Penitipan')
                            {{ date('d M Y', strtotime($bayar->tanggal_masuk)) }}-{{ date('d M Y', strtotime($bayar->tanggal_keluar)) }}
                            @else
                                Tanggal : {{ date('d M Y', strtotime($bayar->tanggal_grooming)) }}
                            @endif
                        </td>
                        <td>{{ $bayar->nama_hewan }}</td>
                        <td style="text-align: left">@currency($bayar->harga_jasa)</td>
                    </tr>
                    <?php $i++; ?>
                @endforeach
            @endif
